...
class RoleService:
    ...
    @staticmethod
    async def seed_default_subscription_roles_and_privileges(db: AsyncSession) -> None:
        """
        Seeds default privileges and subscription tier roles (Normal, Pro, Ultra, Admin)
        if they do not already exist in the database.
        """
        default_privilege_definitions = [
            ("feature:standard_chat", "Standard Chat Access", 1),
            ("feature:quiz_generation", "Quiz Generation", 2),
            ("limit:daily_sessions_5", "5 Daily Sessions Limit", 3),
            ("feature:visual_mode", "Visual Learning Mode", 4),
            ("feature:export_pdf", "Export Sessions to PDF", 5),
            ("limit:daily_sessions_50", "50 Daily Sessions Limit", 6),
            ("feature:multi_agent_tutor", "Multi-Agent Tutor Access", 7),
            ("feature:unlimited_sessions", "Unlimited Daily Sessions", 8),
            ("feature:priority_execution", "Priority Execution Queue", 9),
        ]

        # Seed Privileges
        existing_p_stmt = select(Privilege)
        p_res = await db.execute(existing_p_stmt)
        existing_privileges = {p.code: p for p in p_res.scalars().all()}

        for code, name, order_number in default_privilege_definitions:
            if code not in existing_privileges:
                priv = Privilege(code=code, name=name, order_number=order_number)
                db.add(priv)
                existing_privileges[code] = priv
        ...
        role_privilege_mappings = {
            ...
            "Ultra": [
                ...
                "feature:visual_mode",
                "feature:export_pdf",
                "feature:multi_agent_tutor",
                "feature:unlimited_sessions",
                "feature:priority_execution",
            ],
            "Admin": list(existing_privileges.keys()),
        }

        existing_r_stmt = select(Role).where(Role.retired == False)
        r_res = await db.execute(existing_r_stmt)
        existing_roles = {r.name: r for r in r_res.scalars().all()}

        for role_name, priv_codes in role_privilege_mappings.items():
            privs = [existing_privileges[code] for code in priv_codes if code in existing_privileges]
            if role_name not in existing_roles:
                role = Role(name=role_name, privileges=privs)
                db.add(role)
                logger.info(f"Seeded default role: '{role_name}' with {len(privs)} privileges.")
            else:
                role = existing_roles[role_name]
                if not role.privileges:
